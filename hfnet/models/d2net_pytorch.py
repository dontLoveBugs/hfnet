...
# -*- coding: utf-8 -*-
"""
@Time    : 2019-10-01 21:42
@Author  : Wang Xin
@Email   : wangxin_buaa@163.com
@File    : d2net_pytorch.py
"""
...
from hfnet.utils.exception import EmptyTensorError
...
class D2NetFrontend:
    ...
    def run(self, image, device='cuda:0'):
        H, W, _ = image.shape

        image = self.pre_process(image)
        image = torch.from_numpy(image[np.newaxis, :, :, :]).to(device)

        banned = None

        dense_features = self.net.dense_feature_extraction(image)
        _, _, h, w = dense_features.size()

        detections = self.net.detection(dense_features)
        ...
        displacements = self.net.localization(dense_features)[0]
        ...
        """
            convert d2net to superpoint output.
        """
        pts = keypoints
        descriptor_map = dense_features

        ret = {'keypoints': pts[:2].T.astype(np.int),
               'local_descriptor_map': np.rollaxis(descriptor_map, 0, 3),
               'local_descriptors': descriptors.T,  # D X N
               'dense_scores': None,
               'scores': None}
        return ret
